...
class Cnpj extends Model {
	...
	protected $fields = [
		"idcnpj", "idperson", "desccnpj","desresponsible", "desramo", "idtypeuser", "desemail", "despassword", "dtregister",
	];
...
*/
	public static function listAll()
	{
		...
		$results= $sql->select("SELECT * FROM tb_cnpjs a INNER JOIN tb_persons b USING(idperson)
		 ORDER BY b.desperson;");

		return $results;
	}

	public function get($idcnpj)
	{

		$sql = new Sql();

		$results = $sql->select("SELECT * FROM tb_cnpj a INNER JOIN tb_persons b USING(idperson) WHERE a.idcnpj = :idcnpj", array(
			":idcnpj"=>$idcnpj
		));
	...
	{
		$sql = new Sql();
		$results = $sql->select("CALL sp_users_save(:desperson, :idtypeuser, :descnpj, :desresponsible, :desramo, :despassword, :desemail, :nrphone)", [
			":desperson" => utf8_encode($this->getdesperson()),
			":idtypeuser" =>2,
            ":descnpj" =>$this->getdescnpj(),
            ":descnpj" =>$this->getdesresponsible(),
            ":descnpj" =>$this->getdesramo(),
